...
function iconByName(station, map_grade, level, mapZoom) {
    ...
    var iconSize = map_grade == "country" ? 'iconSize20' : 'iconSize30',
        ...
        html = '';
    if (mapZoom <= 5) {
        html = '<b class="iconSize ' + iconSize + '" style="background-image: url('
            + iconUrl + ');"><div class="tooltip_country_name">' + station.name
            + '<span></span></div></b></div>'
    } else {
        iconSize = 'iconSize30',
            iconUrl = '../images/marker/mark_sprite_1.png';
...
function popWindowset(station, map_grade) {
    layer.open({
        ...
        content: '<div class="leaflet-popup-content" style="width: 800px;">' +
        '<div class="loading_popup_msg">加载中...</div></div>',
