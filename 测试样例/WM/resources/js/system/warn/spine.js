...
function getList(){
	tab = $('#tab').DataTable({
        "ajax": {
        	 url:$.coreApiPath+"/spine/query/list",
        	 type: 'get',
        	 data : function (d) {
        ...
        "columns" : [
                     ...
                     { "title":"均值倍数","data":"avgMultiple","className":"text-center"},
                     { "title":"浓度阈值","data":"threshold","className":"text-center"},
...
function update(id){
	$("#addModal").modal("show");
		$(".modal-title").text("尖刺告警    / 修改")
	$.ajax({
        type: "get",
        url:$.coreApiPath+"/spine/query/id/"+id,
        error: function(request) {
        },
        success: function(data) {
