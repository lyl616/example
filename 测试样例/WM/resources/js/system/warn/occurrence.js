...
function getList() {
	tab = $('#tab').DataTable({
		"ajax": {
			url: $.coreApiPath + "/occurrence/query/list",
			type: 'get',
			data: function(d) {
			...
			},
			{
				...
				"render": function(data, type, full, meta) {
					...
					if(isShowUpdateBtn == 1) {
						up = '<button class="btn btn-warning" onclick=update("' + data + '")>修改</button>';
					}
					return up;
				}
			}
...
function add() {
	$.ajax({
		type: "POST",
		url: $.coreApiPath + "/occurrence/save",
...
function update(id) {
	$("#addModal").modal("show");
	$(".modal-title").text("阈值告警    / 修改")
	$.ajax({
		type: "get",
		url: $.coreApiPath + "/occurrence/query/id/" + id,
		error: function(request) {},
		success: function(data) {
	...
	layer.msg('你确定要删除吗？', {
		...
		yes: function(index) {
			...
			$.ajax({
				...
				async: false,
				dataType: "json",
				url: $.coreApiPath + "/occurrence/del",
				data: {
					ids: ids
				},
				success: function(data) {
