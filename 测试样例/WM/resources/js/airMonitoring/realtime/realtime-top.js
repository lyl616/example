...
function init_switchTal() {
    $("#polutionTab a").removeClass("btn-white");
    $("#polutionTab a").addClass("btn-white");
    $("#polutionTab a").css("cursor", 'pointer');
    switchTab($("#polutionTab a"));
}

function remove_switchTab() {
    var type_a = $("#polutionTab a");
    type_a.off("click");
    for (var i in type_a) {
        if (type_a[i].id == 'pm10') {
            ...
            var src = $.ctx + "/resources/img/legend/legend-" + currentPolutionType + ".png";
...
function get24HourTP10AqiData(type) {
    ...
    var url = $.coreApiPath + "/realtime/get24AqiData";
    var postData = {
        city: $("#city").val(),
...
function getNowHourTP10AqiData(type) {
    ...
    var url = $.coreApiPath + "/realtime/getNowTP10AqiData";
    var postData = {
        city: $("#city").val(),
        pollution_type: currentPolutionType,
        station_type: type
...
function mkTop10Html(listAqiData) {
    var tempHtml = "";
    for (var i = 0; i < 10; i++) {
        var td1 = "<td class=\"tc\"><span class=\"top_3\">" + (i + 1) + "</span></td>"
        if (typeof(listAqiData[i]) != "undefined") {
            ...
            if (currentPolutionType.indexOf("aqi") != -1) {
                var aqiFirstd = getAqiFirst(listAqiData[i].aqiFirst);
                var id = 'top' + i;
                ...
                tempHtml += "<tr onclick='showStationInfo(\"" + listAqiData[i].stationId + "\")'>" + td1 +
                    ...
                    "<td class=\"text-center\">" + aqiFirstd + "</td></tr>";
            } else {
                tempHtml += "<tr onclick='showStationInfo(\"" + listAqiData[i].stationId + "\")'>" + td1 +
...
function mkEnd10Html(listAqiData) {
    ...
    for (var i = 0; i < 10; i++) {
        ...
        if (typeof(listAqiData[i]) != "undefined") {
            var td1 = "<td class=\"tc\"><span class=\"top_other\">" + (i + 1) + "</span></td>"
            if (i == 0) {
                td1 = "<td class=\"tc\"><span class=\"top_1\">" + (i + 1) + "</span></td>"
            } else if (i == 1) {
                td1 = "<td class=\"tc\"><span class=\"top_2\">" + (i + 1) + "</span></td>"
            } else if (i == 2) {
                td1 = "<td class=\"tc\"><span class=\"top_3\">" + (i + 1) + "</span></td>"
            }
            var pvalue = listAqiData[i].pvalue;
            var level = getLevalByValAndType(pvalue, currentPolutionType);
            var fontcolor = "#0c1726";
...
function getAqiFirst(aqiFirst) {

    if (aqiFirst == null || aqiFirst == "") {
        return "";
    } else {
        if (aqiFirst.indexOf(",") == -1) {
            return showUpper(aqiFirst);
        } else {
            var arr = aqiFirst.split(",");
            var data = [];
            $.each(arr, function (i, val) {
                data.push(showUpper(val));
            })
            return data.join(",");
