...
function tooltip_axis_windy() {
    return {
        ...
        formatter: function (params) {
            ...
            data[0] = new Date(params[0].value[dims.time]).Format("yyyy-MM-dd HH:mm");
            ...
            for (var i = 4; i < keys.length; i++) {
                ...
                for (var j = 0; j < params.length; j++) {
                    ...
                    if (stName == seriesName) {
                        dt = {
                            'text': stName,
                            'value': params[j].value[dims[kk]],
                            'tip': params[j].value[keys.length + dims[kk]] == "-" ? "" : params[j].value[keys.length + dims[kk]],
            ...
            sortArr = dataSortWind(sortArr);


            for (var j = 0; j < sortArr.length; j++) {
                str += '<div class="tooltip-data" style="width:120px;display: inline">' + sortArr[j].marker + sortArr[j].text + ":" + sortArr[j].value + sortArr[j].tip + "</div>";
            }
            str += '</div>';
            return str;
        },
...
function dataZoom_start_end(len) {
    return [
        {
            textStyle: {
                color: '#8392A5'
