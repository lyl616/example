...
function makeStationDetailTitle(wsStation) {
    var dis = "";
    if (wsStation.district) {
        dis = filterUnLegalStr(wsStation.district)
    }
    var sid = wsStation.stationId;
    var addr = filterUnLegalStr(wsStation.address);
    var stationType = filterUnLegalStr(wsStation.type);
    var stationName = filterUnLegalStr(wsStation.stationName);
    return stationType + ' ' + stationName + ' [' + sid + '] ' + dis + ' ' + addr;
}

function filterUnLegalStr(str) {
...
function validatePwd(pwd) {
    if (pwd == "" || pwd == null || pwd.trim() == "") {
        layer.msg("密码为能为空！");
        return false;
    }
    var reg = /^(?=.*[A-Z])(?=.*\d)[^]{8,50}$/;
    if (!reg.test(pwd)) {
        layer.msg("密码格式不对，长度为8-50位，且必须包含大写字母和数字");
        return false;
    }
    return true;
}
