...
$(function() {
	...
	function touchS() {
		windMap.stop();
		$("canvas.leaflet-tile-container").css("display", 'none');
	}

	function touchE() {
		windMap.update();
		$("canvas.leaflet-tile-container").css("display", 'block');
	}
...
function addStationMarkers() {
	...
	if(station_data[map_grade][polutionType] == null) {
		$.ajax({
			...
			beforeSend: initLayerLoaderMsg("加载"), // 发送请求
...
function iconByName(station, map_grade, level, mapZoom) {
	...
	var iconSize = map_grade == "country" ? 'iconSize20' : 'iconSize30',
		iconUrl = $.ctx + '/resources/img/country/' + map_grade + level + '.png',
		html = '';
	if(mapZoom <= 5) {
		...
		iconSize = 'iconSize30',
			iconUrl = $.ctx + '/resources/img/country/city' + level + '.png';
...
function popWindowset(station, map_grade) {
	...
	var areaType = null,
		id = station.id;
	if(map_grade == "country") { //城市级别
		areaType = 1;
	} else { //站点级别
		areaType = 2;
	}
	//城市&站点。弹窗请求
	var pop_url = 'waming://com.waming-air.app/stationDetail?id='+id+"&areaType="+areaType;
...
function initMap(map, url) { //地图的版权信息
	L.tileLayer(url, {
		zoom: 11,
		touchZoom: true, //是否支持手指的滑动来放大缩小地图
...
function getMapDataIp(callback) {
    $.getJSON($.backendApiPath + "/config/windy", {}, function (json) {
        if (json.erroCode == 2000) {
            mapDataIp = json.result;
            if (callback)
                callback(mapDataIp);
        }
    })
...
function createWindMap(map, windycolor) {
	...
	function windyD3(mapDataIp) {
		while(mapDataIp.indexOf('"') != -1) {
			mapDataIp = mapDataIp.replace('"', '');
		}
...
function selectdown_click(obj) {
	var selVle = obj.id;
	$("#select_down .select_down-selector").removeClass("active");
	$("#select_down " + "#" + selVle).addClass("active");
	var imgurl = $.ctx + '/resources/beta_app/images/leafletMapIcon/legend/wm-legend-' + selVle + '.png';
...
var initWebPage = {
	initLegend: function() {
		...
		legend.onAdd = function(map) {
			...
			div.innerHTML = '<img id="img_polltype" src="' + $.ctx + '/resources/beta_app/images/leafletMapIcon/legend/wm-legend-aqi.png" />';
			return div;
		};
		legend.addTo(map);
	},
	...
	InitRightMenu: function() {
		...
		function pollution_cut(value) {
			//			other_click_hide();
			//相关站点的操作
			//toggleMenuClass('pollution_type');
			$('.select_down').show();
		}
		L.control.custom({
			position: 'bottomright',
			content: '<div class="menu_btn"><i class="menu_icon menu_wrw" id="pollution_type"></i>' +
				//				'	<div class="btn_div bg_gray" ><span id="pollution_val">AQI</span><i class="icon_react"></i></div>' +
				...
				'		</div>' +
				'	</div>' +
				'<div class="menu_btn"><i class="menu_icon menu_cz_hide" id="d_value"></i><div class="tip_msg">该功能即将上线</div></div>' +
				'<div class="menu_btn"><i class="menu_icon menu_zd" id="station_info"></i></div>' +
				'<div class="menu_btn"><i class="menu_icon menu_fc" id="windy_field"></i></div>',
			...
			events: {
				click: function(data) {
					$('.select_down').hide();
					$('.tip_msg').hide();
					if(data.target.id == 'd_value') { //差值
						data.target.nextSibling.style.display = 'block';
					} else if(data.target.id == 'pollution_type') { //污染物
						pollution_cut('AQI');
...
function restStatiion_html(data) {
	...
	var html = '';
	html += '<div class="pop_st_baseinfo">';
	...
	data.insTime = datasplit(data.insTime);
	html += "    " + new Date(data.insTime).Format("yyyy-MM-dd HH:mm");
	html += '       <span class="pull-right" onclick="aloseAllLayer()">X</span>';
	html += '	</div>';
	html += '<div style="height:418px; overflow-x:hidden; overflow-y:scroll;">';
	html += '	<ol class="pop_st_ol col-xs-12">';
	html += '		<li class="col-xs-4">质量等级:' + data.grade + '</li>';
	html += '		<li class="col-xs-4">质量级别: ' + level + '</li>';
	html += '		<li class="col-xs-12">首要污染物：:' + aqiFirst + '</li>';
	html += '	</ol>';

	html += '	<table cellpadding="0" cellspacing="0" class="table table-bordered table-striped">';
	html += '		<tr class="even">';
	html += '			<td>AQI</td>';
	...
	html += '           </select>';
	html += '			<button  class="btn btn-xs btn-white m-l-10 bf08a" onclick="showEcharts(1,this,\'hourlyChart\')">浓度</button>';
	...
	html += '		<div id="hourlyChart" class="positionAbs opacity1 zindex1" style="width: 100%; height: 300px;"></div>';
	html += '		<div id="particulateChart" class="positionAbs opacity0 zindex0" style="width: 100%; height: 300px;"></div>';
	html += '		<div id="gaseousChart" class="positionAbs opacity0 zindex0" style="width: 100%; height: 300px;"></div>';
	html += '	</div>';
	html += '</div>';
	return html;
}
