...
(function() {
	...
	var OVERLAY_TYPES = {
		...
		"ox": {
			...
			scale: "log",
			precision: 0,
			label: "光化学オキシダント Photochemical Oxidants",
			unit: " ppb",
			multiplier: 1000
		},
		"so2": {
		...
		"nmhc": {
			...
			scale: "log",
			precision: 2,
			label: "非メタン炭化水素 Non-Methane Hydrocarbons",
			unit: " ppm"
		},
		"spm": {
			min: 1,
			...
			scale: "log",
			precision: 0,
			label: "浮遊粒子状物質 Suspended Particulate Matter",
		...
		"pm25": {
			...
			scale: "log",
			precision: 0,
			label: "微小粒子状物質 2.5µm Particulate Matter",
			unit: ' μg/m<span class="sup">3</span>'
		}
	};
	...
	var log = {
		...
		info: function(s) {
			if(console && console.info) console.info(s);
		},
		error: function(e) {
			if(console && console.error) console.error(e.stack ? e + "\n" + e.stack : e);
		},
		time: function(s) {
			if(console && console.time) console.time(s);
		},
		timeEnd: function(s) {
			if(console && console.timeEnd) console.timeEnd(s);
		}
	};
	...
	function createAlbersProjection(lng0, lat0, lng1, lat1, view) {
		...
		var s = 1 / Math.max((p1[0] - p0[0]) / view.width, (p0[1] - p1[1]) / view.height) * 1.3;
		// Move the center to (0, 0) in pixel space.
		var t = [view.width / 4, view.height / 1.8];
	...
	function intervalTime(settings) {
		var totalHour;
		var currentIndex = -1;
		var payInternal = 2000;
		totalHour = d3.select("#display").attr("data-total-hour");
		if(totalHour == "0") {
			return;
		}
		var choDate = d3.select("#display").attr("data-date");
		var startDate = new Date(choDate);
		var timer1 = window.setInterval(autoplayAir, payInternal);
		...
		function autoplayAir() {
			...
			var seltime = startDate.getTime() + currentIndex * 60 * 60 * 1000;
			var param = getWindyParam(seltime);
			var datePath = $.coreApiPath + "/data/wind/" + param.year + "/" + param.m + "/" + param.day + "/" + param.hour;
			dataTask = loadJson(datePath);
			fieldTask = when.all([dataTask, settingsTask, renderTask]).then(apply(interpolateField));
			d3.select(FIELD_CANVAS_ID).remove();
			d3.select("#field").html(['<canvas id="field-canvas"></canvas>']);
			d3.select(FIELD_CANVAS_ID).attr("width", view.width).attr("height", view.height);
			animateTask = when.all([settingsTask, fieldTask]).then(apply(animate));
		}
	...
	function buildMeshes(topo, settings) {
		...
		var outerBoundary = topojson.mesh(topo, topo.objects.main, function(a, b) {
			return a === b;
		});
		var divisionBoundaries = topojson.mesh(topo, topo.objects.main, function(a, b) {
			return a !== b;
		});
	...
	function animate(settings, field) {
		...
		for(var i = 0; i < settings.particleCount; i++) {
			particles.push(field.randomize({
				age: rand(0, settings.maxParticleAge)
			}));
		}
	...
	function report(e) {
		log.error(e);
		displayStatus(null, e.error ? e.error == 404 ? "无数据" : e.error + " " + e.message : e);
	}
	...
	var intervalTimeTask = when.all([settingsTask]).then(apply(intervalTime));
