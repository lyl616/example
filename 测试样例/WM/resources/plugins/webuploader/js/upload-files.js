    ...
    $.fn.extend({
        ...
        diyUpload: function (opt, serverCallBack) {
            ...
            webUploader.on('uploadProgress', function (file, percentage) {
                ...
                //超过数量隐藏上传按钮
                if ($("li[id^='fileBox']").length >= opt.fileNumLimit) {
                    $(".upload-pick").hide();
                }
            });
        ...
        showHaveFile: function (fileId, fullUrl, wmUrl, removeCallback, isImage, fileName) {
            ...
            //添加子容器;
            var li = '<li id="fileBox_' + fileId + '" class="diyUploadHover"> \
					<div class="viewThumb">\
					    <input type="hidden">\
					    <div class="diyBar"> \
							<div class="diyProgress">' + fileName + '</div> \
					    </div> \
            ...
            var $fileBox = $parentFileBox.find('#fileBox_' + fileId);
            var $diyBar = $fileBox.find('.diyBar');
            if (!fileName) {
                $diyBar.hide();
            }
            ...
            var $diyCancel = $fileBox.find('.diyCancel').on('click', function () {
                layer.confirm('确认删除' + fileName + '文件吗？', {
                    btn: ['是', '否'] //按钮
                }, function () {
                    $(".upload-pick").show();
                    layer.closeAll();
            ...
            //获取文件后缀类型
            var extType = getFileExtType(wmUrl);
            if (isImage) {
                $fileBox.find('.viewThumb').append('<img alt="' + fileName + '" class="viewerImgClass" data-original="' + fullUrl + '" src="' + fullUrl + '" >');
            } else {
                if (extType == 'img') {
                    $fileBox.find('.viewThumb').append('<a href="' + fullUrl + '" target="_blank"><img title="' + fileName + '" src="' + fullUrl + '" ></a>');
                } else {
                    $fileBox.find('.viewThumb').append('<a href="' + fullUrl + '" target="_blank"><img title="' + fileName + '" src="' + $.ctx + '/resources/plugins/webuploader/images/' + extType + '.png" ></a>');
                }
            }
        }
    });
    ...
    function createBox($fileInput, file, webUploader, opt) {
        ...
        var li = '<li id="fileBox_' + file_id + '" class="diyUploadHover"> \
					<div class="viewThumb">\
					    <input type="hidden">\
					    <div class="diyBar"> \
							<div class="diyProgress">' + file.name + '</div> \
					    </div> \
					    <p class="diyControl"><span class="diyLeft"><i></i></span><span class="diyCancel"><i></i></span><span class="diyRight"><i></i></span></p>\
					</div> \
				</li>';
        ...
        //绑定取消事件;
        var $diyCancel = $fileBox.find('.diyCancel').on('click', function () {
            layer.confirm('确认删除' + file.name + '吗？', {
                btn: ['是', '否'] //按钮
            }, function () {
                layer.closeAll();
                $(".upload-pick").show();
                removeLi($("#fileBox_" + file_id), file_id, webUploader);
                if (opt.removeCallback) {
                    opt.removeCallback(file);
                }
            }, function () {
                layer.closeAll();
            });
        ...
        //获取文件后缀类型
        var extType = getFileExtType(file.name);

        //生成预览缩略图;
        webUploader.makeThumb(file, function (error, dataSrc) {
            if (!error) {
                $fileBox.find('.viewThumb').append('<img alt="' + file.name + '" class="viewerImgClass" src="' + dataSrc + '" >');
            } else {
                $fileBox.find('.viewThumb').append('<a href="javascript:void(0);" target="_blank"><img title="' + file.name + '" src="' + $.ctx + '/resources/plugins/webuploader/images/' + extType + '.png" ></a>');
            }
        });
    }
