(function e(t, n, r) {
	...
	3: [function(require, module, exports) {
		...
		exports.default = {
			props: {
				...
				paginationInfoTemplate: {
					type: String,
					default: function _default() {
						return "显示第 {from} 到 {to}项结果，共 {total} 项";
					}
				},
			...
			methods: {
					...
					this.fields.forEach(function(field, i) {
						if(typeof field === 'string') {
							...
							obj = {
								...
								titleClass: field.titleClass === undefined ? 'text-center' : field.titleClass,
								dataClass: field.dataClass === undefined ? 'text-center' : field.dataClass,
				...
				setPagination: function(total) {
					var tablePagination = {};
					tablePagination.current_page = this.currentPage;
					tablePagination.total = total;
					tablePagination.per_page = this.perPage;
					tablePagination.from = (this.currentPage - 1) * this.perPage + 1;
					tablePagination.to = this.currentPage * this.perPage > total ? total : this.currentPage * this.perPage;
					tablePagination.total = total;
					tablePagination.last_page = Math.ceil(total / this.perPage);
				...
				loadData: function loadData() {
					...
					var url = this.apiUrl;
					var data = {};
					var plist = (this.getAllQueryParams().split('&') || []);
					for(var i = 0; i < plist.length; i++) {
						data[plist[i].split('=')[0]] = plist[i].split('=')[1];
					}
					...
					//            this.$http.post(url, data, this.httpOptions).then(function (response) {
					$.ajax(options).then(function(response) {
						// 成功回调
						if(response.erroCode == '2000') {
							self.tableData = self.getObjectValue(response.result, self.dataPath, null);

							self.tablePagination = self.setPagination(response.result.recordsTotal);
				...
				getAllQueryParams: function getAllQueryParams() {
					var params = [this.queryParams.sort + '=' + this.getSortParam(), this.queryParams.page + '=' + (this.currentPage - 1), this.queryParams.perPage + '=' + this.perPage].join('&');
				...
				callCallback: function callCallback(field, item) {
					...
					if(typeof this.$parent[func] == 'function') {
						if(field.callbackitem) {
							return this.$parent[func].apply(this.$parent, [item]);
						} else {
				...
				onRowChanged: function onRowChanged(dataItem, itemNumber) {					
					dataItem.itemNumber=itemNumber;					
				...
				onMouseEnter: function onMouseEnter(dataItem, field, event) {
                    			this.$dispatch(this.eventPrefix + 'cell-mouseenter', dataItem, field, event);
                		}, 
				onMouseLeave: function onMouseLeave(dataItem, field, event) {
                    			this.$dispatch(this.eventPrefix + 'cell-mouseleave', dataItem, field, event);
                		},
			...
			events: {
                ...
                'vuetable:clearTable': function vuetableRefresh() {
                    this.tableData = {};
                    this.tablePagination = this.setPagination(0);
                }
			},
		...
		var vuetablehtml = '<div class="{{wrapperClass}}">' +
			'	<table class="vuetable {{tableClass}}" style="margin-bottom: 0px; */">' +
			'			<colgroup>' +
			...
			'					<template v-if="field.visible">' +
			'						<template v-if="isSpecialField(field.name)">' +
			'							<th v-if="extractName(field.name) == \'__checkbox\'" :class="[field.titleClass, \'checkbox_\'+extractArgs(field.name)]"> <input type="checkbox" @change="toggleAllCheckboxes($event.target.checked, field.name)" :checked="checkCheckboxesState(field.name)"> </th>' +
			'                          <th v-if="extractName(field.name) == \'__component\'" @click="orderBy(field, $event)" class="{{field.titleClass || \'\'}} {{isSortable(field) ? \'sortable\' : \'\'}}"> {{field.title || \'\'}} <i v-if="isCurrentSortField(field) &amp;&amp; field.title" class="{{ sortIcon(field) }}" :style="{opacity: sortIconOpacity(field)}"></i> </th>' +
			'							<th v-if="notIn(extractName(field.name), [\'__checkbox\', \'__component\'])" id="{{field.name}}" class="{{field.titleClass || \'\'}}"> {{field.title || \'\'}} </th>' +
			'						</template>' +
			'						<template v-else="">' +
			'							<th @click="orderBy(field, $event)" id="_{{field.name}}" class="{{field.titleClass || \'\'}} {{isSortable(field) ? \'sortable\' : \'\'}}"> {{getTitle(field) | capitalize}}&nbsp; <i v-if="isCurrentSortField(field)" class="{{ sortIcon(field) }}" :style="{opacity: sortIconOpacity(field)}"></i> </th>' +
			'						</template>' +
			'					</template>' +
			'				</template>' +
			'			</tr>' +
			...
			'				<colgroup @dblclick="onRowDoubleClicked(item, $event)" @click="onRowClicked(item, $event)" :render="onRowChanged(item,itemNumber)" :class="onRowClass(item, itemNumber)">' +
			'					<template v-for="field in fields">' +
			'						<template v-if="field.visible">' +
			'							<template v-if="isSpecialField(field.name)">' +
			'								<col width="100" v-if="extractName(field.name) == \'__sequence\'" class="vuetable-sequence {{field.dataClass}}" v-html="tablePagination.from + itemNumber"> </td>' +
			'								<col width="100" v-if="extractName(field.name) == \'__checkbox\'" class="vuetable-checkboxes {{field.dataClass}}"> <input type="checkbox" @change="toggleCheckbox($event.target.checked, item, field.name)" :checked="rowSelected(item, field.name)"> </col>' +
			'								<col width="100" v-if="field.name == \'__actions\'" class="vuetable-actions {{field.dataClass}}">' +
			'									<template v-for="action in itemActions"> <button class="{{ action.class }}" @click="callAction(action.name, item)" v-attr="action.extra">                                            <i class="{{ action.icon }}"></i> {{ action.label }}                                        </button> </template>' +
			'								</col>' +
			'								<col width="100" v-if="extractName(field.name) == \'__component\'" class="{{field.dataClass}}">' +
			'									<component :is="extractArgs(field.name)" :row-data="item" :row-index="itemNumber"></component>' +
			'								</col>' +
			'							</template>' +
			'							<template v-else="">' +
			'								<col width="100" v-if="hasCallback(field)" class="{{field.dataClass}}" @click="onCellClicked(item, field, $event)" @dblclick="onCellDoubleClicked(item, field, $event)"> {{{ callCallback(field, item) }}} </col>' +
			'								<col width="100" v-else="" class="{{field.dataClass}}" @click="onCellClicked(item, field, $event)" @dblclick="onCellDoubleClicked(item, field, $event)"> {{{ getObjectValue(item, field.name, "") }}} </col>' +
			'							</template>' +
			'						</template>' +
			'					</template>' +
			'				</colgroup>' +
			...
			'						<colgroup v-if="isVisibleDetailRow(item[detailRowId])" @click="onDetailRowClick(item, $event)" :transition="detailRowTransition" :class="[detailRowClass]">' +
			'							<col width="100" :colspan="countVisibleFields">' +
			'								<component :is="detailRowComponent" :row-data="item" :row-index="itemNumber"></component>' +
			'							</col>' +
			'						</colgroup>' +
			'					</template>' +
			'					<template v-else="">' +
			'						<colgroup v-if="isVisibleDetailRow(item[detailRowId])" v-html="callDetailRowCallback(item)" @click="onDetailRowClick(item, $event)" :transition="detailRowTransition" :class="[detailRowClass]"></colgroup>' +
			'					</template>' +
			'				</template>' +
			'			</template>' +
			'		<tbody v-cloak="">' +
			'			<template v-for="(itemNumber, item) in tableData">' +
			'				<tr @dblclick="onRowDoubleClicked(item, $event)" @click="onRowClicked(item, $event)" :render="onRowChanged(item,itemNumber)" :class="onRowClass(item, itemNumber)">' +
			'					<template v-for="field in fields">' +
			'						<template v-if="field.visible">' +
			'							<template v-if="isSpecialField(field.name)">' +
			'								<td v-if="extractName(field.name) == \'__sequence\'" class="vuetable-sequence {{field.dataClass}}" v-html="tablePagination.from + itemNumber"> </td>' +
			'								<td v-if="extractName(field.name) == \'__checkbox\'" class="vuetable-checkboxes {{field.dataClass}}"> <input type="checkbox" @change="toggleCheckbox($event.target.checked, item, field.name)" :checked="rowSelected(item, field.name)"> </td>' +
			...
			'							</template>' +
			'							<template v-else="">' +
			'								<td v-if="hasCallback(field)" class="{{field.dataClass}}" @mouseenter="onMouseEnter(item, field, $event)" @mouseleave="onMouseLeave(item, field, $event)" @click="onCellClicked(item, field, $event)" @dblclick="onCellDoubleClicked(item, field, $event)"> {{{ callCallback(field, item) }}} </td>' +
			'								<td v-else="" class="{{field.dataClass}}" @mouseenter="onMouseEnter(item, field, $event)" @mouseleave="onMouseLeave(item, field, $event)" @click="onCellClicked(item, field, $event)" @dblclick="onCellDoubleClicked(item, field, $event)"> {{{ getObjectValue(item, field.name, "") }}} </td>' +
			'							</template>' +
			'						</template>' +
			'					</template>' +
			'				</tr>' +
			...
			'		<div class="vuetable-pagination-info {{paginationInfoClass}}" v-html="paginationInfo"> </div>' +
			...
			'	</div>' +
			'</div>';
		(typeof module.exports === "function" ? module.exports.options : module.exports).template = vuetablehtml;
		if(module.hot) {
			(function() {
				module.hot.accept()
